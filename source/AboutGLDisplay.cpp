...
AboutGLDisplay::AboutGLDisplay(HDC hDC_, int clientWidth_, int clientHeight_) :
		m_glOptions(), glyphMetrics(), hDC(hDC_), outText(), clientWidth(
				clientWidth_), clientHeight(clientHeight_), mode(MoveNone), objectIndex(
				3), objectNumMajor(32), objectNumMinor(32)
...
// Create a set of display lists based on the glyphs of the TT font we selected
	...
	// Create a set of display lists based on the glyphs of the TT font we selected
	if (!(wglUseFontOutlines(hDC, 0, 127, GLF_START_LIST, 0.0f,
			m_glOptions.textExtrusion, WGL_FONT_POLYGONS, glyphMetrics)))
	{
		AnsiString error = AnsiString("wglUseFontOutlines failed!")
...
void AboutGLDisplay::drawTorus(void) const
{
	...
	int numStrips =
			m_glOptions.halfObject ? objectNumMajor / 2 : objectNumMajor;
...
void AboutGLDisplay::setupPalette()  const
{
...

	{
		...
		{
			pPal->palPalEntry[i].peRed = (GLbyte) ((((i >> pfd.cRedShift)
					& redMask) * 255) / redMask);
			pPal->palPalEntry[i].peGreen = (GLbyte) ((((i >> pfd.cGreenShift)
					& greenMask) * 255) / greenMask);
			pPal->palPalEntry[i].peBlue = (GLbyte) ((((i >> pfd.cBlueShift)
